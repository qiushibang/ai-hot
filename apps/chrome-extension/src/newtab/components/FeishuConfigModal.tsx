...
export const FeishuConfigModal = ({ open, settings, onSave, onClose }: Props) => {
  const [draft, setDraft] = useState({
    feishuWebhookUrl: settings.feishuWebhookUrl ?? '',
    feishuAppId: settings.feishuAppId ?? '',
    feishuAppSecret: settings.feishuAppSecret ?? '',
    feishuReceiveId: settings.feishuReceiveId ?? ''
  })
  const [presetName, setPresetName] = useState('')
  const [presets, setPresets] = useState<FeishuPreset[]>(loadFeishuPresets)
  const [saving, setSaving] = useState(false)
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null)

  const update = (f: keyof typeof draft, v: string) => {
    setDraft((p) => ({ ...p, [f]: v }))
    setMessage(null)
  }

  const fields = [
    { key: 'feishuWebhookUrl' as const, lbl: 'Webhook URL（备选）' },
    { key: 'feishuAppId' as const, lbl: 'App ID' },
    { key: 'feishuAppSecret' as const, lbl: 'App Secret', password: true },
    { key: 'feishuReceiveId' as const, lbl: 'Receive ID', placeholder: 'oc_ 群聊 / ou_ 私聊' }
  ]
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = {
        ...settings,
        feishuWebhookUrl: draft.feishuWebhookUrl || null,
        feishuAppId: draft.feishuAppId || null,
        feishuAppSecret: draft.feishuAppSecret || null,
        feishuReceiveId: draft.feishuReceiveId || null
      }
  ...
  const handleSavePreset = () => {
    ...
    if (!name) return
    setPresets(saveFeishuPreset(name, {
      feishuWebhookUrl: draft.feishuWebhookUrl || null,
      feishuAppId: draft.feishuAppId || null,
      feishuAppSecret: draft.feishuAppSecret || null,
      feishuReceiveId: draft.feishuReceiveId || null
  ...
  const handleLoadPreset = (p: FeishuPreset) => {
    setDraft({
      feishuWebhookUrl: p.feishuWebhookUrl ?? '',
      feishuAppId: p.feishuAppId ?? '',
      feishuAppSecret: p.feishuAppSecret ?? '',
      feishuReceiveId: p.feishuReceiveId ?? ''
    })
    setMessage(null)
  }

  const handleDeletePreset = (id: string) => {
    setPresets(deleteFeishuPreset(id))
  }

  return (
    <Modal open={open} title="飞书推送配置" onClose={onClose}>
      {fields.map((f) => (
